...
class NeuralNetwork2(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_classes):
        ...
        self.embedding = nn.Embedding(vocab_size, embedding_dim)
        self.lstm = nn.LSTM(embedding_dim, hidden_size=64, num_layers=1, batch_first=True)  # Add LSTM layer
        self.fc1 = nn.Linear(64, 128)  # Adjust input dimension to match LSTM output
        self.fc2 = nn.Linear(128, 32)
        self.fc3 = nn.Linear(32, num_classes)
    def forward(self, x):
        embedded = self.embedding(x)
        lstm_out, (hidden, cell) = self.lstm(embedded)  # Pass embedded inputs to LSTM
        x = torch.relu(self.fc1(lstm_out[:,-1,:]))  # Use last LSTM output as input to FC layers
        x = torch.relu(self.fc2(x))
        x = torch.softmax(self.fc3(x), dim=1)
        return x
...
class IntentModel2(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_classes):
        super(IntentModel2, self).__init__()
        self.embedding = nn.Embedding(vocab_size, embedding_dim)
        self.conv1d = nn.Conv1d(in_channels=embedding_dim, out_channels=128, kernel_size=3, padding=1)  # Increase output channels, added padding
        self.pool = nn.MaxPool1d(kernel_size=2)
        self.lstm = nn.LSTM(128, hidden_size=128, num_layers=2, batch_first=True, dropout=0.2)  # Increase hidden size, add another layer, add dropout
    ...
    def forward(self, x):
        ...
        conv_out = torch.relu(self.conv1d(embedded))
        pooled = self.pool(conv_out)
        lstm_out, _ = self.lstm(pooled.permute(0, 2, 1))  # Removed cell output as it's not used
        lstm_out = lstm_out[:, -1, :]  # Taking only the last output
...
class Attention(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        context = attention.transpose(1, 2).bmm(encoder_outputs)
        return context
...
class NeuralNetwork(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_classes):
        ...
        self.pool = nn.MaxPool1d(kernel_size=2)
        self.lstm = nn.LSTM(128, hidden_size=128, num_layers=20, batch_first=True, dropout=0.2)
        self.attention = Attention(128)  # Attention mechanism
        self.fc1 = nn.Linear(256, 64)  # Adjusted input size to match concatenated output
        self.fc2 = nn.Linear(64, num_classes)
    ...
    def forward(self, x):
        ...
        conv_out = torch.relu(self.conv1d(embedded))
        pooled = self.pool(conv_out)
        lstm_out, _ = self.lstm(pooled.permute(0, 2, 1))
        
        # Apply attention mechanism
        attention_weights = self.attention(lstm_out[:, -1, :], lstm_out)
        context = attention_weights.squeeze(1)
        lstm_out = torch.cat((context, lstm_out[:, -1, :]), dim=1)
        
        lstm_out = torch.relu(self.fc1(lstm_out))
        x = torch.softmax(self.fc2(lstm_out), dim=1)
        return x
...
class IntentModel():
    ...
    def train2(self):
        ...
        self.label_encoder = lbl_encoder
